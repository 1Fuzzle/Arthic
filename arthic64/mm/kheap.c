...
/* Payloads are 16-byte aligned now, not 8.
 *
 * The 64-bit ABI expects 16 for anything that might hold a wide type, and
 * malloc is conventionally aligned for the widest type the platform has. On
 * 32-bit that was 8; here it is 16, and returning less would be a trap for
 * whoever uses this later.
 */
/* Original note on why alignment matters at all: Misaligned accesses are merely slow on x86, but
...
void kheap_init(void)
{
	...
	uint64_t base = pmm_alloc_frames(HEAP_FRAMES);

	if (!base) {
		kprintf("kheap: could not reserve %lu KB\n", (uint64_t)(HEAP_SIZE / 1024));
		return;
	}
...
void kfree(void *ptr)
{
	...
	if (b->magic != BLOCK_MAGIC) {
		kprintf("kfree: bad or corrupted block at 0x%lx - ignoring\n",
		        (uint64_t) ptr);
		return;
	}

	if (b->free) {
		kprintf("kfree: double free at 0x%lx - ignoring\n", (uint64_t) ptr);
		return;
	}
...
void kheap_stats(uint64_t *total, uint64_t *used, uint64_t *blocks)
{
	uint64_t u = 0, n = 0;
