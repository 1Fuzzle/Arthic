 ...
 * ASLR
 *
 * USER_STACK_TOP is the ceiling of the stack arena, not the address any
 * program actually uses anymore. Each program's real stack top is picked
 * randomly within ASLR_STACK_SLACK bytes below that ceiling, so two runs of
 * the same program get different stack addresses. That value lives per-
 * program in struct program.stack_top and is what the rest of the code
 * (syscall range, the jump into ring 3) actually uses.
 */
...
struct program {
	uint32_t page_dir;
	uint32_t entry;
	uint32_t stack_top;   /* ASLR: randomized per-program, see loader_run(). */
...
static void program_task(void)
{
	...
	syscall_set_user_range(USER_LOAD_ADDR, prog->stack_top);
	...
	usermode_jump(prog->entry, prog->stack_top - 16);
}
...
int loader_run(const char *name, const char *args)
{
	...
	/* ASLR: pick this program's stack top somewhere in the last
	 * ASLR_STACK_SLACK bytes below the arena ceiling, page-aligned. Running
	 * the same program twice in a row now gives two different addresses. */
	uint32_t slack_pages  = ASLR_STACK_SLACK / PAGE_SIZE;
	uint32_t stack_offset = rand_range(slack_pages) * PAGE_SIZE;
	prog->stack_top       = USER_STACK_TOP - stack_offset;

	uint32_t stack_bottom = prog->stack_top - USER_STACK_SIZE;
	...
	kprintf("loader: %s, %u bytes, entry 0x%x, stack top 0x%x, %u segments, own address space\n",
	        name, size, elf->entry, prog->stack_top, prog->segments);
